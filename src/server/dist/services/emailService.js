...
class EmailService {
    ...
    async sendVerificationEmail(to, verifyUrl, role = 'user') {
    ...
    async sendMail(to, subject, html) {
        try {
            ...
            console.error('❌ Fout bij versturen e-mail:', error.response?.data || error.message);
            return false;
        }
    }
    async sendWelcomeEmail(to, name, type) {
        const subject = type === 'retailer'
            ? 'Welkom bij FiT - Retailer Account'
            : 'Welkom bij FiT';
        const html = type === 'retailer'
    ...
    async sendPaymentSuccessEmail(to, name, planType) {
        try {
            await this.sendMail(to, 'Betaling Succesvol - FiT Account Geactiveerd', this.getPaymentSuccessTemplate(name, planType));
    ...
    async sendFitSessionCompleteEmail(to, name) {
        try {
            await this.sendMail(to, 'Je FiT Sessie is Voltooid!', this.getFitSessionCompleteTemplate(name));
    ...
    async sendPasswordResetEmail(to, resetLink) {
        try {
            ...
            const sent = await this.sendMail(to, 'Wachtwoord Reset - FiT by Brendr.io', html);
    ...
    async testConnection() {
        try {
            ...
            console.error('❌ Brevo API connection failed:', err.response?.data || err.message);
            return false;
        }
    }
    async sendFitSessionCompletedEmail(to, name) {
        return this.sendFitSessionCompleteEmail(to, name);
    }
    async sendSubscriptionSuccessEmail(email, firstName, planName) {
    ...
    async sendPaymentFailedEmail(email, firstName) {
    ...
    getRetailerWelcomeTemplate(name) {
        return `
    ...
    getPaymentSuccessTemplate(name, planType) {
        return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    getFitSessionCompleteTemplate(name) {
        return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
