...
async function includedFromConfigOwner(plan) {
    const fallback = (pl) => {
        if (pl === 'BASIC')
            return 500;
        if (pl === 'PREMIUM')
...
router.get('/categories', auth_1.requireAdmin, async (_req, res) => {
    try {
        let cats = null;
        try {
...
router.post('/categories/:key/hero', auth_1.requireAdmin, catUpload.single('hero'), async (req, res) => {
    try {
...
router.get('/categories/:key', auth_1.requireAdmin, async (req, res) => {
    try {
        const key = String(req.params.key || '').toUpperCase();
...
router.patch('/categories/:key/status', auth_1.requireAdmin, async (req, res) => {
    try {
        const key = String(req.params.key || '').toUpperCase();
...
router.patch('/categories/:key/settings', auth_1.requireAdmin, async (req, res) => {
    try {
        const key = String(req.params.key || '').toUpperCase();
...
router.patch('/categories/:key/promo', auth_1.requireAdmin, async (req, res) => {
    try {
        const key = String(req.params.key || '').toUpperCase();
...
router.get('/consumers', auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        if (userTypeRaw && ['USER', 'ADMIN'].includes(userTypeRaw))
            query = query.eq('user_type', userTypeRaw);
        if (hideGuests)
        ...
        if (error)
            throw error;
        const userIds = (rows || []).map((u) => u.id).filter((id) => !!id);
        const statsByUser = {};
        if (userIds.length > 0) {
            try {
                const { data: sessionRows } = await supabase_1.supabaseAdmin
                    .from('fit_sessions')
                ...
                (sessionRows || []).forEach((s) => {
                    ...
                    if (s.status === 'PROCESSING')
                        bucket.processingSessions += 1;
                    if (s.satisfied === true)
                        bucket.satisfiedTrueSessions += 1;
                    if (s.satisfied === false)
                        bucket.satisfiedFalseSessions += 1;
                });
            }
            catch { }
        }
...
router.get('/fit-settings', auth_1.requireAdmin, async (_req, res) => {
    try {
        const { data: sys } = await supabase_1.supabaseAdmin
            .from('category_settings')
            .select('settings')
...
router.put('/fit-settings', auth_1.requireAdmin, async (req, res) => {
    try {
...
router.post('/fit-settings/apply-all-users', auth_1.requireAdmin, async (req, res) => {
    try {
...
router.get('/subscription-settings', auth_1.requireAdmin, async (_req, res) => {
    try {
        const { data: sys } = await supabase_1.supabaseAdmin
            .from('category_settings')
            .select('settings')
...
router.put('/subscription-settings', auth_1.requireAdmin, async (req, res) => {
    try {
        const body = req.body || {};
...
router.post('/subscription-settings/apply-included', auth_1.requireAdmin, async (_req, res) => {
    try {
        const { data: sys } = await supabase_1.supabaseAdmin
            .from('category_settings')
            .select('settings')
...
router.get('/retailers', auth_1.requireAdmin, async (req, res) => {
    try {
        const page = Math.max(parseInt(String(req.query.page || '1'), 10) || 1, 1);
        const limit = Math.min(Math.max(parseInt(String(req.query.limit || '20'), 10) || 20, 1), 100);
        const offset = (page - 1) * limit;
        ...
        if (error)
            throw error;
        const retailerIds = (rows || []).map((r) => r.id).filter((id) => !!id);
        const sessionsByRetailer = {};
        const shopsByRetailer = {};
        const activePlanByRetailer = {};
        if (retailerIds.length > 0) {
            ...
            try {
                const { data: shopRows } = await supabase_1.supabaseAdmin
                    .from('shops')
                    .select('retailer_id')
                    .in('retailer_id', retailerIds);
                (shopRows || []).forEach((s) => {
            ...
            try {
                const { data: subRows } = await supabase_1.supabaseAdmin
                    .from('subscriptions')
        ...
        const items = await Promise.all((rows || []).map(async (r) => {
            ...
            try {
                if (r.auth_id) {
                    const au = await supabase_1.supabaseAdmin.auth.admin.getUserById(String(r.auth_id));
            ...
            const effectivePlanType = activePlanByRetailer[r.id] != null ? activePlanByRetailer[r.id] : (r.plan_type || null);
            return {
                id: r.id,
...
router.get('/shops', auth_1.requireAdmin, async (req, res) => {
    try {
        const page = Math.max(parseInt(String(req.query.page || '1'), 10) || 1, 1);
        const limit = Math.min(Math.max(parseInt(String(req.query.limit || '20'), 10) || 20, 1), 100);
        const offset = (page - 1) * limit;
        ...
        if (regTo)
            query = query.lte('created_at', normalizeEnd(regTo));
        if (retailerEmail)
            query = query.eq('retailers.email', retailerEmail);
        ...
        const filtered = items.filter(it => {
            if (!Number.isFinite(sessionsMin) && !Number.isFinite(sessionsMax))
                return true;
            const c = it.totalSessions || 0;
            if (Number.isFinite(sessionsMin) && c < sessionsMin)
                return false;
            if (Number.isFinite(sessionsMax) && c > sessionsMax)
                return false;
            return true;
        });
...
router.patch('/retailers/:id/plan', auth_1.requireAdmin, async (req, res) => {
    try {
        const retailerId = String(req.params.id || '').trim();
        const bodyPlan = req.body?.planType;
        const plan = typeof bodyPlan === 'string' ? bodyPlan.toUpperCase() : '';
        const allowed = ['STARTER', 'BASIC', 'PREMIUM', 'ENTERPRISE'];
        if (!retailerId || !allowed.includes(plan)) {
            res.status(400).json({ success: false, message: 'Ongeldige retailer of planType' });
            return;
        }
        const { data: retailer, error: rErr } = await supabase_1.supabaseAdmin
            .from('retailers')
            .select('id, plan_type')
            .eq('id', retailerId)
            .maybeSingle();
        ...
        const currentPlan = String(retailer.plan_type || 'STARTER').toUpperCase();
        if (currentPlan === plan) {
            res.json({ success: true, data: { retailerId, planType: plan } });
            return;
        }
        const included = await includedFromConfigOwner(plan);
        const { error: updRetailerErr } = await supabase_1.supabaseAdmin
            .from('retailers')
            .update({ plan_type: plan, updated_at: new Date().toISOString() })
            .eq('id', retailerId);
        if (updRetailerErr)
            throw updRetailerErr;
        try {
            ...
            console.warn('[owner] set plan subscriptions sync failed (non-fatal):', e?.message || e);
        }
...
router.get('/sessions', auth_1.requireAdmin, async (req, res) => {
    try {
      ...
      `, { count: 'exact' });
        ...
        if (userType === 'guest')
            query = query.eq('users.is_guest', true);
        if (userType === 'logged')
            query = query.eq('users.is_guest', false);
        if (satisfied === 'true')
        ...
        if (error)
            throw error;
        const sessionIds = (rows || []).map((r) => r.id).filter((id) => !!id);
        const productBySession = {};
        if (sessionIds.length > 0) {
            try {
                const { data: prodRows } = await supabase_1.supabaseAdmin
